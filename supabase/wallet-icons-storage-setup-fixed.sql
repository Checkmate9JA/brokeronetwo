-- Storage Bucket Setup for Wallet Icons (Fixed Version)
...
-- 3. Create RLS policies for the wallet-icons bucket
...
-- 4. Create a helper function to generate file paths for wallet icons
...
-- 5. Create a function to validate wallet icon uploads
...
-- 6. Drop existing table if it exists and recreate
DROP TABLE IF EXISTS public.wallet_icon_files CASCADE;

-- 7. Create a table to track wallet icon files
...
CREATE INDEX idx_wallet_icon_files_wallet_id ON public.wallet_icon_files(wallet_id);
CREATE INDEX idx_wallet_icon_files_user_email ON public.wallet_icon_files(user_email);
CREATE INDEX idx_wallet_icon_files_is_active ON public.wallet_icon_files(is_active);

-- 9. Create RLS policies for the wallet_icon_files table
ALTER TABLE public.wallet_icon_files ENABLE ROW LEVEL SECURITY;

-- Drop existing policies if they exist
DROP POLICY IF EXISTS "Users can view their own wallet icon files" ON public.wallet_icon_files;
DROP POLICY IF EXISTS "Users can insert their own wallet icon files" ON public.wallet_icon_files;
DROP POLICY IF EXISTS "Users can update their own wallet icon files" ON public.wallet_icon_files;
DROP POLICY IF EXISTS "Users can delete their own wallet icon files" ON public.wallet_icon_files;
DROP POLICY IF EXISTS "Admins can view all wallet icon files" ON public.wallet_icon_files;
DROP POLICY IF EXISTS "Admins can update all wallet icon files" ON public.wallet_icon_files;
DROP POLICY IF EXISTS "Admins can delete all wallet icon files" ON public.wallet_icon_files;
...
-- 10. Create a trigger to update the updated_at column
DROP TRIGGER IF EXISTS update_wallet_icon_files_updated_at ON public.wallet_icon_files;
...
-- 12. Create a function to get public URLs for wallet icons
...
-- 13. Create a function to deactivate old wallet icons when a new one is uploaded
...
-- 14. Create trigger to automatically deactivate old wallet icons
DROP TRIGGER IF EXISTS deactivate_old_wallet_icons_trigger ON public.wallet_icon_files;
...
DO $$
BEGIN
  RAISE NOTICE 'Wallet icons storage setup completed successfully!';
  RAISE NOTICE 'Bucket: wallet-icons';
  RAISE NOTICE 'Table: wallet_icon_files';
  RAISE NOTICE 'Policies: Created for storage.objects and wallet_icon_files';
END $$;
